...
## 5. Copy The Bot To The Pi

From PowerShell on your Windows PC, run this from the folder that contains `TunezBot`:

```powershell
scp -r .\TunezBot your_pi_username@raspberrypi.local:~/TunezBot
```

If you need to use the Pi IP address:

```powershell
scp -r .\TunezBot your_pi_username@192.168.1.123:~/TunezBot
```
...
Make sure `.env` exists on the Pi:

```bash
ls -la
```

If `.env` did not copy, create it:
...
If you edit the bot on your Windows PC, copy it to the Pi again, then on the Pi:
